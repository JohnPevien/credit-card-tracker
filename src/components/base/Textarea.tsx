...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, error, helpText, variant = 'bordered', textareaSize, className = '', ...props }, ref) => {
    ...
    const textareaClasses = [baseClasses, variantClasses, sizeClasses, errorClasses, className]
      .filter(Boolean)
      .join(' ');
      ...
      <div className="form-control w-full">
        ...
        {(error || helpText) && (
          <div className="label">
            ...
            {helpText && !error && <span className="label-text-alt">{helpText}</span>}
          </div>
        )}
      </div>
    );
